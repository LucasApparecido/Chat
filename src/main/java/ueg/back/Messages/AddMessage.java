...
public class AddMessage {
    ...
    private void mountMessage(String message, String userName, Boolean writer) {
        ...
        if (Objects.equals(userName, "Servidor")) {

            prefix = userName + ": ";
            messageLabel = new JLabel(prefix + message);
            messageLabel.setFont(new Font("Times New Roman", Font.PLAIN, 14));

            messageLabelServer = new JLabel("Um novo usuário entrou no chat!");
            messageLabelServer.setFont(new Font("Times New Roman", Font.BOLD, 14));

            decoratorColorS = new ColorDecorator(serverMessageColor);
            decoratorColorS.decorate(messageLabel);
            decoratorColorS.decorate(messageLabelServer);

            reply.add(messageLabelServer);
